...
def round_to_5_or_10(price):
    """
    Round price to the nearest 5 or 10 upwards if it doesn't end with 0 or 5.
    If price ends with 0 or 5, leave it as is.
    """
    # Check if price already ends with 0 or 5
    last_digit = int(price * 100) % 10
    if last_digit == 0 or last_digit == 5:
        return price
    
    # Round up to nearest 5 or 10
    price_cents = int(price * 100)
    remainder = price_cents % 10
    
    if remainder < 5:
        price_cents = price_cents - remainder + 5
    else:
        price_cents = price_cents - remainder + 10
...
def get_shopify_skus():
    """Get all products with SKUs and prices from Shopify using cursor-based pagination."""
    skus = []
...
def update_shopify_price(variant_id, sku, pos_price, current_shopify_price):
    """
    ...
    # Apply rounding to nearest 5 or 10 if needed
...
def main():
    ...
    for item in shopify_skus:
        ...
        if pos_data is not None:
            ...
            update_shopify_price(variant_id, original_sku, pos_data["price"], current_shopify_price)
        else:
